...
for(i in 1:N)
{
  outDEoptim <- DEoptim(fn=function(x){cec2013(I, x)}, lower, upper, 
                        DEoptim.control(itermax = 100, storepopfrom = 1, storepopfreq = 2, NP = i*10))
  ...
  for( l in 1:10)
  {
    ...
    wynik8<-k_means(tab, k, 100, fn ,D, method ="rand", selection = "classic", punishment = "yes")
    wynik9<-k_means(tab, k, 100, fn ,D, method ="rand", selection = "choice", punishment = "yes")
    wynik10<-k_means(tab, k, 100, fn ,D, method ="rand", selection = "Random", punishment = "yes")
...
colnames(wyniki) <- c("Uzyskane_minimum","Choice","Random","pClassic","pChoice","pRandom","ilosc_osobnikow")

p3<-ggplot(data = wyniki, aes(x=ilosc_osobnikow)) +
  ...
  # geom_point() +
  scale_x_discrete(limits=c(1:N)*10)
